class World {
  ...
  spawnObjects() {
    ...
    for (var dx = -1; dx <= 1; dx++) {
      for (var dy = -1; dy <= 1; dy++) {
        ...
        if (
          posX >= 0 ||
          posX < this.roomsPerRow ||
          posY >= 0 ||
          posY < this.roomsPerRow
        ) {
      ...
      for (var dy = -1; dy <= 1; dy++) {
        if ((dx != 0 || dy != 0) && Math.abs(dx) + Math.abs(dy) < 2) {
          if (this.getRoom(wumpusX + dx, wumpusY + dy) != null) {
              ...
              this.getRoom(wumpusX + dx, wumpusY + dy).stench_cnt++;
          }
        }
      }
    }
    ...
    for (
      var i = 0;
      i < Math.floor((this.roomsPerRow * this.roomsPerRow) / 10);
      i++
    ) {
      ...
      this.getRoom(pitX, pitY).addObject(
        new Pit(createVector(pitX, pitY), this)
      );
      for (var dx = -1; dx <= 1; dx++) {
        for (var dy = -1; dy <= 1; dy++) {
          if ((dx != 0 || dy != 0) && Math.abs(dx) + Math.abs(dy) < 2) {
            if (this.getRoom(pitX + dx, pitY + dy) != null) {
                ...
                this.getRoom(pitX + dx, pitY + dy).breeze_cnt++;
            }
          }
        }
      }
  ...
  getRoom(x, y) {
    if (
      x < 0 ||
      x > this.roomsPerRow - 1 ||
      y < 0 ||
      y > this.roomsPerRow - 1
    ) {
      return null;
    } else {
